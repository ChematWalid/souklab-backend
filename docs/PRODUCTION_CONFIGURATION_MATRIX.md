...
| API rate limiting | `APP_RATE_LIMIT_*`, `REDIS_*` | `app.rate-limit.*` | `AppProperties.RateLimit`, `RateLimitFilter` | Redis-backed Bucket4j state; shared limiter is mandatory in production |
