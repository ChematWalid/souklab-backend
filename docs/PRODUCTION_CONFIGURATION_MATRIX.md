...
| Feed social policy | `FEED_MAX_MEDIA_PER_POST`, `FEED_ALLOWED_IMAGE_MIME_TYPES`, `FEED_MAX_TITLE_LENGTH`, `FEED_MAX_BODY_LENGTH`, `FEED_MAX_COMMENT_LENGTH`, `FEED_MAX_TAGS_PER_POST`, `FEED_MAX_TAG_LENGTH` | `app.feed.*` | `AppProperties.Feed`, feed services and validators | Positive bounded lengths/counts; normalized tags; explicit image MIME allowlist |
