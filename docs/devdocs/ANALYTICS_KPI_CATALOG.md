...
| `averageReportResolutionSeconds` | average `resolved_at - created_at` for resolved or dismissed reports created in range | `content_reports.created_at`, `resolved_at` | soft-deleted and unresolved reports excluded; zero when no report has been resolved |
...
| `engagementByRegion` | distinct event actors grouped by the artisan profile's region slug | `activity_events.actor_id` joined to `users.artisan.region` | only actors with a persisted region are included; respects the optional event-type filter |
| `engagementByCraftCategory` | distinct event actors grouped by the artisan profile's craft-category slug | `activity_events.actor_id` joined to `users.artisan.sub_category.category` | only actors with a persisted craft category are included; respects the optional event-type filter |
