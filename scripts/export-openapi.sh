...
auth_header=()
if [ -n "${SOUKLAB_ACCESS_TOKEN:-}" ]; then
  auth_header=(-H "Authorization: Bearer ${SOUKLAB_ACCESS_TOKEN}")
fi
curl --fail --silent --show-error --location "${auth_header[@]}" "$base_url" --output "$output_file"
